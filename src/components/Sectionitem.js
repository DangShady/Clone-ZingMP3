...
const Sectionitem = ({ link, title, data, thumbnailM, sortDescription, artistsNames }) => {
    ...
    return (
        <div
            ...
            className='flex flex-col gap-2 flex-auto justify-start   w-1/4 text-sm cursor-pointer'
        >
            <div onMouseEnter={handleHover}
                onMouseLeave={handleLeave}
                className='w-full relative overflow-hidden rounded-lg'>
                {isHover && <div className='absolute top-0 bottom-0 z-10 left-0 right-0 bg-overlay-30 rounded-lg text-white items-center flex justify-center gap-3'>
            ...
            <span className='flex flex-col'>
                <span className='font-semibold'>{title?.length >= 20 ? `${title?.slice(0, 20)}...` : title}</span>
                {data?.sectionId === 'h100'
                    ? <span>{artistsNames}</span>
                    : <span>{sortDescription?.length >= 30
                        ? `${sortDescription?.slice(0, 30)}...`
